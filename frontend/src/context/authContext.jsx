...
export const AuthProvider = ({ children }) => {
    ...
    const loadUser = async () => {
      try {
        ...
        if (storedUser && token) {
          ...
          setUser(userData);
          localStorage.setItem(process.env.REACT_APP_USER_KEY || 'softranch_user', JSON.stringify(userData));
        }
      } catch (error) {
  ...
  const register = useCallback(async (userData) => {
    try {
      ...
      localStorage.setItem(process.env.REACT_APP_USER_KEY || 'softranch_user', JSON.stringify(newUser));
      setUser(newUser);
  ...
  const updateProfile = useCallback(async (data) => {
    try {
      ...
      localStorage.setItem(process.env.REACT_APP_USER_KEY || 'softranch_user', JSON.stringify(updatedUser));
      setUser(updatedUser);
