...
export const tokenManager = {
  getAccessToken: () => {
    ...
    return localStorage.getItem(process.env.NEXT_PUBLIC_TOKEN_KEY || 'softranch_access_token')
  },
  getRefreshToken: () => {
    if (typeof window === 'undefined') return null
    return localStorage.getItem(process.env.NEXT_PUBLIC_REFRESH_TOKEN_KEY || 'softranch_refresh_token')
  },
  setAccessToken: (token) => {
    if (typeof window === 'undefined') return
    localStorage.setItem(process.env.NEXT_PUBLIC_TOKEN_KEY || 'softranch_access_token', token)
  },
  setRefreshToken: (token) => {
    if (typeof window === 'undefined') return
    localStorage.setItem(process.env.NEXT_PUBLIC_REFRESH_TOKEN_KEY || 'softranch_refresh_token', token)
  },
  removeTokens: () => {
    if (typeof window === 'undefined') return
...
const api = {
  ...
  blogs: {
    ...
    getBySlug: (slug) => request.get(`/blogs/slug/${slug}`),
  ...
  // Team endpoints
  team: {
    getAll: (params) => request.get('/team', { params }),
    getOne: (id) => request.get(`/team/${id}`),
    create: (data) => request.post('/team', data),
    update: (id, data) => request.patch(`/team/${id}`, data),
    delete: (id) => request.delete(`/team/${id}`),
    getPublic: () => request.get('/team/public'),
  },

  // Contact endpoints
  contact: {
    send: (data) => request.post('/contact', data),
    getAll: (params) => request.get('/contact', { params }),
    getOne: (id) => request.get(`/contact/${id}`),
    markAsRead: (id) => request.patch(`/contact/${id}/read`),
  ...
  analytics: {
    ...
    getPageViews: (params) => request.get('/analytics/pageviews', { params }),
    getTraffic: (params) => request.get('/analytics/traffic', { params }),
    getBlogStats: () => request.get('/analytics/blogs'),
  },
}
